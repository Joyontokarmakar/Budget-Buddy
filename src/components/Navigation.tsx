...
import { LayoutDashboard, TrendingDown, Wallet, PieChart, Settings, FileText, Gem, Menu, X, Trash2, UserPlus, LogOut, Coins } from 'lucide-react';
...
export const Navigation: React.FC = () => {
  ...
  const getDesktopActiveIndex = () => {
    if (location.pathname === '/') return 0;
    if (location.pathname === '/expenses') return 1;
    if (location.pathname === '/accounts') return 2;
    if (location.pathname === '/deposits-loans') return 3;
  ...
  return (
    <>
      ...
      >
          ...
          <div className="mb-4 bg-muted/40 border border-border/60 rounded-2xl p-3 relative">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-[10px] font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-lg hover:bg-primary/20 active:scale-[0.95] transition-all"
              >
        ...
        <div className="mt-6 pt-4 border-t border-border/50 text-xs text-muted-foreground/70 font-medium text-center space-y-1.5">
          <div className="flex items-center justify-center gap-1.5 flex-wrap">
            <span>Budget Buddy Student v1.0.0</span>
            <span>•</span>
            <NavLink to="/about" onClick={() => setIsMoreOpen(false)} className="hover:text-primary transition-colors underline font-semibold">
              {t('nav.about') || 'About'}
            </NavLink>
          </div>
          <div className="text-[11px] text-muted-foreground/60">
            Built by <span className="font-semibold text-foreground">Joyonto Karmakar</span>
            {' • '}
            <a
      ...
      <aside className="fixed top-0 bottom-0 left-0 z-40 hidden w-64 border-r border-border bg-card/50 backdrop-blur-md p-6 md:flex flex-col">
        ...
        <div className="flex items-center gap-3 mb-8 px-2">
          ...
          <div className="flex flex-col">
            <span className="font-bold text-sm tracking-tight">Budget buddy</span>
        ...
        {/* Footer Credit */}
        <div className="pt-4 border-t border-border/50 text-xs text-muted-foreground/70 font-medium text-center space-y-1.5">
          <div className="flex items-center justify-center gap-1.5 flex-wrap">
            <span>Budget Buddy Student v1.0.0</span>
            <span>•</span>
            <NavLink to="/about" className="hover:text-primary transition-colors underline font-semibold">
              {t('nav.about') || 'About'}
            </NavLink>
          </div>
          <div className="text-[11px] text-muted-foreground/60">
